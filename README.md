...
Este Script foi desenvolvido para otimizar e agilizar o processo de inserção da foto do estudante no cadastro de alunos do sistema SIGEDUCA. Ele adiciona um botão 'Tirar foto', abaixo do botão 'Selecionar foto', na interface que permite habilitar a webcam e capturar a foto do aluno em tempo real, eliminando a necessidade de tirar a foto em outro aplicativo, salvar no computador e depois fazer o upload manual no sistema.
...
* **JavaScript:** Lógica de captura e manipulação da interface.
* **HTML/CSS:** Estruturação e estilização do botão.
* **WebRTC / MediaDevices API:** Para comunicação nativa com a webcam.
